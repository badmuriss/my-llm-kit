...
4. Write the three files under `openspec/changes/<slug>/`. Every task needs a stable ID, exact paths, context, acceptance criteria, and one `Check:` line. Use one executable plus arguments; put shell pipelines in a reviewed script. Use `Check: missing validation evidence` when no real command is known.

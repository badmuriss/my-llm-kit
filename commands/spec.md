...
4. Write the three files under `openspec/changes/<slug>/`. Every task needs a stable ID, exact paths, context, acceptance criteria, and one `Check:` line. Load `frontend-visual-validation` for rendered UI. Every changed surface and state needs the complete matrix as `Visual: <id> | <route-or-component> | <platform> | <width>x<height> | <state>`. Use one executable plus arguments; put shell pipelines in a reviewed script. Use `Check: missing validation evidence` when no real command is known.
